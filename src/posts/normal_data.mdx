...
<Output>
	KL divergence of mean brightness 0.02243918311004641 KL divergence of single pixel
	0.6474992150590142
</Output>
...
The solution is simple. While estimating the _probability density_ function requires smoothing the data, it's much easier to estimate the _cumulative distribution_ function. It only involves sorting our data. We can estimate the distribution function $\hat F$ by:
...
Using this CDF we propose a very simple statistic giving the distance between two distributions. If $\hat F$ is the estimated CDF, and we're comparing it to the CDF $G$, then we define the _Kolmogorov-Smirnov_ statistic by
...
<Output>
	Kolmogorov-Smirnov statistic for Mean brightnes: 0.0315, with p-value 1.361E-36 Kolmogorov-Smirnov
	statistic for single pixel: 0.2608, with p-value 0.000E+00
</Output>
